...
export class QuestionUI {
    ...
    private _renderStep(step: number): void {
        ...
        // Input dan hint hanya muncul di step terakhir
        this.polyaInput.style.display = isLastStep ? 'block' : 'none';
        this.btnHint.style.display = isLastStep ? 'inline-flex' : 'none';
        ...
        if (this._hintVisible && isLastStep) {
            this._showHint();
        } else {
            this.feedbackArea.textContent = '';
            this.feedbackArea.className = 'q-feedback';
    ...
    private readonly _handleSubmit = (): void => {
        ...
        // Step bukan terakhir — langsung lanjut tanpa input
        if (!isLastStep) {
            this._renderStep(this._currentStep + 1);
            return;
        }

        // Step terakhir — validasi input
        ...
        this._validateFinalAnswer(parseFloat(raw));
    };
    ...
    private _validateFinalAnswer(value: number): void {
        if (value === this._question!.jawaban) {
            this._handleCorrect();
        } else {
            this._handleWrong();
        }
    ...
    private _handleWrong(): void {
        ...
        if (this._attempts >= MAX_ATTEMPTS) {
            this._gaveUp = true;
            this.polyaInput.disabled = true;
            this._showFeedback('error', `❌  Jawaban yang benar adalah ${this._question!.jawaban}. Semangat untuk soal berikutnya!`);
            this.btnSubmit.textContent = 'Lewati →';
            this.btnSubmit.disabled = false;
            this.btnHint.disabled = true;
        } else {
            const remaining = MAX_ATTEMPTS - this._attempts;
            this._showFeedback('error', `❌  Belum tepat. Coba lagi! (${remaining}× kesempatan tersisa)`);
            this.polyaInput.value = '';
    ...
    private _emitSkipped(): void {
        EventBus.emit(GameEvent.QUESTION_SKIPPED, { questionId: this._question!.id });
        EventBus.emit(GameEvent.QUESTION_ANSWERED, {
            questionId: this._question!.id,
            correct: false,
    ...
    private _triggerStarAnimation(starCount: number): void {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const sizePx = 22 + Math.random() * 16;
    ...
    private static _injectStyles(): void {
        ...
        style.textContent = /* css */`
            ...
            #question-backdrop.question-visible { opacity: 1; }
            ...
            .q-polya-input.q-wiggle { animation: q-wiggle 0.28s ease both; }
            ...
            .q-polya-input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
            .q-polya-input:focus {
                border-color: #2d7a3a;
                box-shadow: 0 0 0 3px rgba(45, 122, 58, 0.18);
                background: #fff;
            }
            .q-polya-input:disabled { opacity: 0.55; cursor: not-allowed; }
            .q-polya-input.q-input-correct { border-color: #2e7d32; background: #e8f5e9; color: #1b5e20; }
            .q-polya-input.q-input-error   { border-color: #c62828; background: #fdecea; color: #b71c1c; }
            ...
            .q-feedback:not(:empty) { padding: 10px 14px; }
            .q-feedback-correct { background: #e8f5e9; color: #1b5e20; font-weight: 700; font-size: 15px; text-align: center; }
            .q-feedback-error   { background: #fdecea; color: #b71c1c; }
            .q-feedback-warning { background: #fff8e1; color: #e65100; }
            .q-feedback-hint    { background: #fff8e1; color: #5d4037; display: flex; gap: 8px; align-items: flex-start; }
            ...
            .q-btn:disabled { opacity: 0.45; cursor: not-allowed; transform: none; }
            .q-btn:focus-visible { outline: 3px solid #2d7a3a; outline-offset: 3px; }
            .q-btn-hint {
                flex: 0 0 auto; padding: 13px 14px;
                background: #fff8e1; color: #8d6e00;
                border: 2px solid #ffe082; font-size: 14px;
            }
            ...
            @keyframes q-star-float {
                ...
                18%  { opacity: 1; transform: translateY(-24px) scale(1.2) rotate(8deg); }
                100% { opacity: 0; transform: translateY(-130px) scale(0.7) rotate(25deg); }
            }
            .q-star {
